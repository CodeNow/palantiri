 ...
 */
RabbitMQ.prototype.publishASGCheckCreated = function (data) {
  data = put({
    timestamp: new Date(),
    dockerHealthCheckId: uuid()
  }, data)

  RabbitMQ._dataCheck(data, ['orgName', 'githubId', 'createdAt'])
  log.info(data, 'RabbitMQ publishASGCheckCreated')
  this.hermesClient.publish('asg.check-created', data)
}
