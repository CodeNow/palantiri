...
const DockLost = require('../../../lib/workers/dock.remove').task
...
describe('dock.remove unit test', function () {
  ...
  it('should call publishEvent for host', function (done) {
    ...
    DockLost(testJob).asCallback(function (err) {
      ...
      sinon.assert.calledWith(rabbitmq.publishEvent, 'dock.remove', testJob)
      done()
    })
  })
})
